...
const PokemonCommentSection:FC<{pokemonId: string}> = async({pokemonId}) => {
  ...
  <div className="max-w-2xl mx-auto px-4">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg lg:text-2xl font-bold text-gray-900 dark:text-white">{t("comments.comments")}: {comments.length}</h2>
    </div>
  <PokemonCommentForm pokemonId={pokemonId} userId={session?.user.id}></PokemonCommentForm>
    ...
    }) : <h3 className='dark:text-white text-black text-2xl text-center py-12'>{t("comments.no-comments")}</h3>}
  </div>
</section>
  )
}
